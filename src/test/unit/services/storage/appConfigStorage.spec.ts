...
describe("appConfigStorage", () => {
  ...
  it("sanitizes persisted system input config and keeps custom shortcuts", async () => {
    mocked.store.get.mockResolvedValue({
      preferences: {
        ...
        systemInput: {
          ...
          doubleTapIntervalMs: 90,
          translateSelectionShortcut: "   ",
          translateClipboardShortcut: " Ctrl+Shift+2 ",
          pasteLastTranslationShortcut: "",
          toggleEnabledShortcut: " Alt+4 ",
          appBlacklist: [" notepad.exe ", null, " ", 1],
          appWhitelist: [" WINWORD.EXE "],
          sourceLanguage: "",
          targetLanguage: " English ",
          onlySelectedText: "yes",
          autoReplace: "no",
          replaceSelectionOnShortcutTranslate: "no",
          enableClipboardFallback: false,
          showFloatingHint: "show",
          onlyWhenEnglishText: false,
          excludeCodeEditors: "keep",
          debugLogging: true,
          captureMode: "broken-mode",
          writebackMode: "simulate-input",
        },
      },
      models: [
        {
    ...
    expect(result.preferences.systemInput).toEqual(
      expect.objectContaining({
        enabled: false,
        triggerMode: createDefaultSystemInputConfig().triggerMode,
        doubleTapIntervalMs: 120,
        ...
        toggleEnabledShortcut: "Alt+4",
        appBlacklist: ["notepad.exe"],
        appWhitelist: ["WINWORD.EXE"],
        sourceLanguage: createDefaultSystemInputConfig().sourceLanguage,
        targetLanguage: "English",
        onlySelectedText: createDefaultSystemInputConfig().onlySelectedText,
        autoReplace: createDefaultSystemInputConfig().autoReplace,
        replaceSelectionOnShortcutTranslate:
          createDefaultSystemInputConfig().replaceSelectionOnShortcutTranslate,
        enableClipboardFallback: false,
        showFloatingHint: createDefaultSystemInputConfig().showFloatingHint,
        onlyWhenEnglishText: false,
        excludeCodeEditors: createDefaultSystemInputConfig().excludeCodeEditors,
        debugLogging: true,
        captureMode: createDefaultSystemInputConfig().captureMode,
        writebackMode: "simulate-input",
      }),
    );
